...
MemoryManager::MemoryManager()
    ...
    while(1) {
        ...
        if (choice != '1' && !isMemorySizeSet) {
            ...
            freeBlock_head = FreeBlock::init_freeBlock(memSize);
        }
        switch(choice){
...
// load memSize from console input and lock memory size
void MemoryManager::setMemorySize() {
    if (isMemorySizeSet) {
        cerr << endl << "warning : u can only set once memory size." << endl;
        return ;
    }
    ...
    while (1) {
        ...
        cin >> memSize;
        if (memSize <= 0)
            cerr << endl << "error : total memory size should be a positive number" << endl;
        else {
            freeMem = memSize;
            isMemorySizeSet = true;
            freeBlock_head = FreeBlock::init_freeBlock(memSize);
...
// rearrange linked block of free blocks according to the order
void MemoryManager::rearrange(int order = 0) {
    // implicit announcement
    if (!order) order = ma_algorithm;

    FreeBlock *start = freeBlock_head;
    FreeBlock *curr, *next, *target;
...
void MemoryManager::selectMemoryAllocationAlgorithm() {
    ...
    while (1) {
        cin >> maa;
        if (maa > 3 || maa < 1)
            cerr << endl << "Please choose 1-3" << endl;
        else {
            cout << "aceess" << endl;
            return ;
        }
...
// create new process accordign to console input
...
// return 1 when creation  successed
int MemoryManager::newProcess() {
    static int successed_times = 0;
    ...
    ab->pid = (1 + successed_times) % MAX_PROCESS_ID;
    ab->next = nullptr;
...
int MemoryManager::retrenchMemory(AllocatedBlock *ab){
    ...
    freeBlock_head = FreeBlock::init_freeBlock(freeMem, usedMem);
    allocateMemory(ab);
...
// create free block
int MemoryManager::freeMemoey(AllocatedBlock* ab) {
    FreeBlock *fb = FreeBlock::init_freeBlock(ab->size, ab->start_addr);
