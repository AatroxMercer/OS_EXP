 ...
 struct free_block_type
{
    ...
    int start_addr;  //空闲块起始位置
     free_block_type *next;  //指向下一个空闲块
};
//指向内存中空闲块链表的首地址
 free_block_type *free_block= NULL;
//每个进程分配到的内存块的描述
 struct allocated_block
{
    ...
    char process_name[PROCESS_NAME_LEN];  //进程名
     allocated_block *next;   //指向下一个进程控制块
};
//进程分配内存块链表的首指针
...
int do_exit();
int allocate_FF( allocated_block *ab);
int allocate_BF( allocated_block *ab);
int allocate_WF( allocated_block *ab);
int allocate( free_block_type *pre, free_block_type *allocate_free_nlock, allocated_block *ab);
int mem_retrench( allocated_block *ab);

//通过内存紧缩技术给新进程分配内存空间
int mem_retrench( allocated_block *ab)
{
     allocated_block *allocated_work,*allocated_pre= allocated_block_head;
     free_block_type *free_work,*free_pre= free_block->next;
...
//给新进程分配内存空间
int allocate( free_block_type *pre, free_block_type *allocate_free_block, allocated_block *ab)
{
...
{
    int ret;
     free_block_type *pre= NULL,*bf= free_block;
    if(bf== NULL)
        return -1;
    while(bf!= NULL)
    {
...
{
    int ret;
     free_block_type *pre= NULL,*ff= free_block;
    if(ff== NULL)
        return -1;
    while(ff!= NULL)
    {
...
int do_exit()
{
     allocated_block *allocated_ab,*allocated_pre;
     free_block_type *free_ab,*free_pre;
    free_pre= free_block;
...
int rearrange_FF()
{
     free_block_type *head= free_block;
     free_block_type *forehand,*pre,*rear;
    int i;
    if(head== NULL)
        return -1;
